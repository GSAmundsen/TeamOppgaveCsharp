...
namespace TeamOppgaveC
{
    internal class Restaurant
    {
        ...
        {
            // Sorterer listen, så bordene med færrest stoler kommer først.
            _tables = _tables.OrderBy(t=>t.GetChairs()).ToList();
            // Lager en liste over alle bord som er store nok, og ledig.
            var availableTable = _tables.Where(t => t.GetChairs() >= guests && !t.IsBooked(resTime)).ToList();
        ...
        public string GetAllReservationsForOneDay(DateTime dtime)
        {
            ...
            var timeSlots = (_closingHour - _openingHour) * 4;
            StringBuilder timeTable = new StringBuilder();
            timeTable.Append("\nBranch: "+_name+"\n\n");
            timeTable.Append(PrintTableHeaders());
            timeTable.Append(PrintTimes(timeSlots, _openingHour));
        ...
        string PrintTimes(int slots, int openingHour)
        {
            StringBuilder result = new StringBuilder();
            DateTime openingTime = new DateTime(1,1,1,openingHour,0,0);            
            
            for(int i = 0; i < slots; i++)
            {
                result.Append("\n" + openingTime.Hour + ':' +  openingTime.ToString("mm"));
